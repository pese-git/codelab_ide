...
class ToolRepositoryImpl implements ToolRepository {
  ...
  @override
  Future<Either<Failure, ToolResult>> executeToolCall(
    ExecuteToolParams params,
  ) async {
    try {
      ...
      return left(Failure.toolExecution(code: e.code, message: e.message));
  ...
  Either<Failure, Unit> validateSafety(ValidateSafetyParams params) {
    try {
      ...
      if (!supportedTools.contains(toolCall.toolName)) {
        return left(
          Failure.validation('Unsupported tool: ${toolCall.toolName}'),
        );
      }
      ...
      if (toolCall.arguments.isEmpty && _requiresArguments(toolCall.toolName)) {
        return left(
          Failure.validation('Tool ${toolCall.toolName} requires arguments'),
        );
      }
  ...
  bool requiresApproval(ToolCall toolCall) {
    ...
    return criticalTools.contains(toolCall.toolName) ||
